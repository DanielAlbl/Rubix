...
function makeCubie(x,y,z) 
{
    ...
    var borders = new THREE.LineSegments(bordersGeo,new THREE.LineBasicMaterial({color: 0xE8E8E8, linewidth: 8}));
...
function moveSide()
{
    ...
    if(prime)
        side.rotateOnAxis(direction, Math.PI/SPEED/2);
    else
        side.rotateOnAxis(direction,-Math.PI/SPEED/2);
...
function cubeRotation()
{
    if(rotation === 0)
        direction.applyQuaternion(orientation);
    cube.rotateOnAxis(direction, Math.PI/SPEED/2);
